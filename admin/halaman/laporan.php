...
// --- Filter dasar: status selesai atau diverifikasi posko ---
$where = "(p.status = 'selesai' OR p.status = 'diverifikasi_posko')";

// --- Filter destinasi ---
if (!empty($destination)) {
    $dest = esc($konek, $destination);
    $where .= " AND t.id = '$dest'";
}
...
$dest_query = mysqli_query($konek, "SELECT id, nama_paket FROM tiket ORDER BY nama_paket ASC");
$dest_list = [];
...
<style>
@media print {
    header, nav, .navbar, .sidebar, .offcanvas, .footer, .no-print {display: none !important;}
    main, .content, .container, body {margin:0!important;padding:0!important;width:100%!important;background:white!important;}
    ...
    thead th {background-color:#212529!important;color:white!important;-webkit-print-color-adjust:exact!important;}
                ...
                <div class="col-12 col-md-3">
                    ...
                    <select name="destination" class="form-select">
                        ...
                        <?php foreach ($dest_list as $d): ?>
                            <option value="<?= $d['id'] ?>" <?= ($destination == $d['id']) ? 'selected' : '' ?>>
